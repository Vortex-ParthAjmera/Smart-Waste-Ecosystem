      ...
      {!event ? (
        <div className="space-y-4">
          ...
          <Card className="border-dashed">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xs text-brand-muted-fg">
                <PlayCircle className="h-4 w-4" /> Dev: trigger demo sequence
              </CardTitle>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-2 p-4 pt-0">
              {fixtures.map((f) => (
                <Button key={f.id} size="sm" variant="outline" onClick={() => injectTestEvent(f.id)}>
                  {f.label}
                </Button>
              ))}
            </CardContent>
        ...
        <div className="space-y-5">
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-wrap gap-2 p-4">
              <span className="w-full text-[10px] font-semibold uppercase text-brand-muted-fg">Dev: fire another event</span>
              {fixtures.map((f) => (
                <Button key={f.id} size="sm" variant="outline" onClick={() => injectTestEvent(f.id)}>
                  {f.label}
                </Button>
