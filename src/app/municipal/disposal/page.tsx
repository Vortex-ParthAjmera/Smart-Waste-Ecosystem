...
export default function ActiveDisposalPage() {
  ...
  // Identity comes from the QR scan the operator just completed, which was
  // verified and resolved server-side. Only the telemetry below still comes
  // from fixtures.
  const scanned = demo.scannedCitizen;
  const displayName = scanned?.name ?? "No citizen scanned";
  const displayLocality = scanned
    ? `${scanned.locality} \u00b7 Tier ${scanned.tier}`
    : "Scan a citizen QR to attach this disposal";

  const event = demo.activeDisposal ?? apiClient.getLatestEventForCitizen(apiClient.DEMO_CITIZEN_ID);
          ...
          <CardContent className="space-y-4 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-brand-muted-fg">{displayLocality}</p>
              </div>
